import { matchGoals, matchScores, ILeaderboard } from '../interfaces/ILeaderboard';
import IGoals from '../interfaces/IGoals';
import { IAwayTeamMatches, IHomeTeamMatches, ITeamGoals } from '../interfaces/ITeam';
...
export default class LeaderboardService implements ILeaderboardService {
  ...
  private static setLeaderboarder(
    team: string,
    name: string,
    matches: ITeamGoals[],
  ) {
  ...
  async getHomeTeamsLeaderboard(): Promise<ILeaderboard[]> {
    ...
    const leaderboard = homeMatches.map(({ name, homeTeamMatches }) =>
      LeaderboardService.setLeaderboarder('homeTeam', name, homeTeamMatches));
  ...
  async getAwayTeamsLeaderboard(): Promise<ILeaderboard[]> {
    ...
    const leaderboard = awayMatches.map(({ name, awayTeamMatches }) =>
      LeaderboardService.setLeaderboarder('awayTeam', name, awayTeamMatches));
